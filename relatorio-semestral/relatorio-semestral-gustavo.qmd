...
da Ruína do Jogador e suas variações. Numa versão simples da original proposta por Pascal [@edwards_pascals_1983], dois jogadores, A e B,
...
Para $q\neq p \neq$, temos $\frac{30}{0.2}=150$. O que condiz com os resultados teóricos.

### Aplicações em casos com regras diferentes

Por si só, o resultado acima não diz muito sobre o estimador, uma vez que podem conferir pelo pequeno valor de
$\left( \frac{1 - (q/p)^a}{1 - (q/p)^N} \right)$ em ([-@eq-original]). Então, passamos a testar casos em que ([-@eq-original])
deixou de funcionar.
...
Aplicando a fórmula original ([-@eq-original]), temos
$$
\mu_a = 150
$$

Isso desconsidera a nova regra de ganho do jogo, o que é refletido na discrepância com o valor simulado de 
$\bar{\psi}_M = 350$
...
as simulações (Apêndice B), que $\mu_a \approx 70.7$. Nessa configuração, $E(X) = 1$ e, pelo estimador,
...
"\$^{a = 10, N = 100, X = \\{+2, -1\\}, p \\textrm{\\ variando\\ a\\ cada\\ } 0.001 \\textrm{\\ no\\ intervalo\\ } (0,1)}\\ \\ \$"),
                 tickfont=6)
  display(analise)
end
main()
...
por DOYLE & SNELL (1984). Queremos descobrir se existem conexões entre o estimador proposto e fórmulas da física, além de outras

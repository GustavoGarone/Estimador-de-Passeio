...
author: "Gustavo Silva Garone, Eduardio Y. G. Ishihara, Elisabeti Kira"
format:
  pdf:
...
de ganho e perca de dinherio dentro do jogo. O jogo termina quando um dos
...
computacional e, por conseguinte, na qualidade dos dados.

Comparação entre o mesmo (ingênuo) algoritmo em Julia e Python*, respectivamente:
...
\**Nota*: Foi utilizado a biblioteca PyCall para criar um ambiente Python no Julia.
A performance pode ter sido afetada minimamente nos tempos de chamada ao Kernel de execução do Python.

Desconsiderando o tempo de compilação (aproximadamente 1.2 segundo do tempo total do código em Julia, que só precisa ser
...
Num caso de o jogador ganhar 2 reais com probabiliade $p=0.3$ e perder 1 real com probabilidade $q = 0.7$, temos a variável
de regra X
$$
X = \begin{cases}
...
Daí conseguimos nosso novo estimador:
$$
\widehat{E}(T_a) = \left\{ \begin{array}{cc}
...
os valores simulados. Isso revela então a vantagem do estimador: conseguimos estimar a duração de um jogo com regras
diversas, o que, até onde encontramos, ainda não foi feito apenas com teoria das probabilidades.
...
Um interessante caminho de abordagem que buscaremos explorar é a conexão entre passeios aleatórios e elétrica como descrito

...
Empregando-se simulações, conseguimos verificar os casos nos quais o estimador é adequado. Exploramos os jogos em que
...
O estimador sob avaliação proposto [@ishihara_um_2025] para a esperança da duração um jogo $J(a,N,X)$ é da seguinte forma
$$
...
Aplicando ([-@eq-estimador]), temos que o valor estimado para a esperança da duração de $J$ é $\widehat{E}(T_J) = \frac{30}{0.2}=150$,
o que condiz com o resultado téorico. Isso nos deu certa segurança para continuar explorando o estimador em jogos mais complexos.
...
O primeiro esboço do algoritmo, implementado na linguagem Python, não satisfez as demandas de tempo e complexidade computacional
que desejávamos, como discutiremos adiante.
Para solucionar essa limitação, migramos para a linguagem Julia, com velocidade computacional competitiva com a linguagem C
...
linguagens Julia e Python. Como previsto por resultados anteriores [@godoy_evaluating_2023], a linguagem Julia mostrou-se
considerávelmente mais rápida do que a linguagem Python, mesmo
levando em consideração o tempo de compilação do código em Julia, que só precisa ser feito uma vez. A adoção da linguagem
Julia foi, portanto, justificada e rapidamente implementada graças à sua similaridade sintática com a linguagem Python.
...
resultado muito próximo do estimado.

## Simulando jogos diversos

Gostaríamos de avaliar se o resultado na seção anterior foi um caso isolado. Para isso, desenvolvemos uma nova versão do
algorítmo que aceitasse regras mais complexas, como variáveis aleatórias com mais que dois resultados possíveis.
Disponibilizamos o código desse novo algoritmo no Apêndice B.

Atravéz desse novo recurso, simulamos jogos sob regras diversas. Nesse contexto, o estimador ([-@eq-estimador])
...
# Código para simulações por um mesmo algoritmo em Julia e Python
...
# Simulação de jogos mais complexos na linguagem Julia para construção da @tbl-simulados
